...
This package provides a **hardware-agnostic platform interface** so that the rest of the verl codebase never calls `torch.cuda.*` (or `torch.npu.*`, ...) directly. Instead, all device-specific logic is routed through a `PlatformBase` singleton obtained via `get_platform()`.
...
The platform is **auto-detected** on first call (CUDA -> NPU -> CPU fallback). Override with the `VERL_PLATFORM` environment variable:
...
Create `verl/plugin/platform/platform_xpu.py` (replace `xpu` with your backend name):
...
No other files need modification. All existing code that uses `get_platform()`, `get_device_name()`, `get_torch_device()`, etc. will automatically pick up the new backend.

### Alternative: Runtime registration (no file edits)

If you prefer not to modify `platform_manager.py`, you can register a custom platform at startup before any other verl code runs:
...
`verl/utils/device.py` is preserved as a thin wrapper. All existing imports like `from verl.utils.device import get_device_name` continue to work - they now delegate to `get_platform()` internally.
