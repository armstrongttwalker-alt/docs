...
此目录实现了 vllm-plugin-FL 的算子调度机制，提供了一个灵活的算子调度系统，根据可用性和策略配置在不同的后端实现（FlagGems、PyTorch、供应商特定）之间进行选择。
...
├── ops.py                   # 后端基础接口
├── discovery.py             # 插件发现机制
├── logger_manager.py        # 集中式日志配置
├── config/                  # 平台特定配置
│   ├── __init__.py          # 配置加载模块
│   ├── ascend.yaml          # 昇腾 NPU 默认配置
│   └── cuda.yaml            # CUDA 默认配置
└── backends/                # 后端实现
    ├── base.py              # 后端抽象基类
    ...
    │       └── custom_attention.py # 注意力后端注册
    ├── reference/           # 参考后端（PyTorch，优先级 50）
    └── vendor/              # 供应商特定后端（优先级 100）
        ├── cuda/            # NVIDIA CUDA 后端
        │   └── impl/
        │       ├── activation.py
        │       ├── normalization.py
...
- `prefer`：首选实现类型
- `strict`：严格模式，当主要实现失败时是否抛出错误
- `per_op_order`：每个算子的自定义选择顺序
- `deny_vendors`：拒绝的供应商列表
- `allow_vendors`：允许的供应商白名单
...
1. **缓存检查**：检查调度缓存是否命中
2. **获取实现**：从注册表中检索所有已注册的实现
3. **供应商过滤**：根据策略的允许/拒绝列表进行过滤
4. **可用性检查**：调用 `is_available()` 检查实现是否可用
5. **优先级排序**：基于每个算子的顺序或默认顺序选择最佳实现
