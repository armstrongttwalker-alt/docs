...
op_backends:
  rms_norm:
    - vendor        # 首先尝试任何可用的供应商
    - flagos        # 然后尝试 flagos
    # reference 未列出，因此不会用于 rms_norm

  silu_and_mul:
    - vendor:cuda   # 仅尝试 CUDA，不尝试其他供应商
    - flagos
    - reference

# FlagGems 算子黑名单（可选）
# 这些算子将不会使用 FlagGems 实现
flagos_blacklist:
  - to_copy
  - zeros
  - mm
...
| `vendor` | 任何可用的供应商后端（自动检测硬件） |
| `vendor:cuda` | 仅 CUDA 供应商后端 |
| `vendor:ascend` | 仅昇腾供应商后端 |

**注意**：使用 `vendor`（不指定供应商名称）时，系统会根据硬件检测自动选择可用的供应商后端。
...
环境变量可以覆盖平台配置中的特定项目。如果未设置，则使用平台配置文件中的值。

### 核心配置

| 变量 | 默认值 | 说明 |
|----------|---------|-------------|
| `VLLM_FL_PREFER_ENABLED` | `true` | 全局开关。设置为 `false` 禁用所有调度功能 |
...
| `VLLM_FL_STRICT` | `0` | 严格模式：`1` = 出错时失败，`0` = 尝试回退 |
| `VLLM_FL_PER_OP` | （无） | 每个算子的顺序：`op1=a\|b\|c;op2=x\|y` |
| `VLLM_FL_ALLOW_VENDORS` | （无） | 供应商白名单，逗号分隔 |
| `VLLM_FL_DENY_VENDORS` | （无） | 供应商黑名单，逗号分隔 |
...
| `FLAGGEMS_ENABLE_OPLIST_PATH` | `/tmp/flaggems_enable_oplist.txt` | FlagGems 启用的算子列表文件 |
...
```{note}
- 白名单和黑名单环境变量互斥（如果同时设置会报错）
- 如果设置了白名单，则完全忽略任何黑名单（环境变量或配置文件）
...
# 结果：
#   prefer: vendor（来自环境变量）
#   flagos_blacklist: [custom_op1, custom_op2]（来自环境变量，配置被忽略）
```

```{note}
- **环境变量是覆盖而非合并**：设置环境变量会完全替换配置值
- **`VLLM_FL_PREFER` 设置的是偏好而非排他性**：它定义了选择顺序，但如果首选后端不可用，会回退到其他后端
- **要强制使用特定后端**：将 `PREFER` 与 `DENY_VENDORS` 结合使用，或使用 `PER_OP` 排除不需要的后端
- **`VLLM_FL_STRICT=1`**：启用严格模式——如果主要实现失败则立即失败，不尝试回退
```

### 回退机制

当 `VLLM_FL_STRICT=0`（默认）时，如果主要实现失败，系统会自动尝试其他可用的实现：

```
算子 'rms_norm' 使用 'default.flagos'（kind=flagos, vendor=None）
[WARNING] 实现 'default.flagos' 对算子 'rms_norm' 失败：...
算子 'rms_norm' 回退到 'reference.torch'（kind=reference, vendor=None）
```
...
| `rms_norm` | RMS 归一化 | ✓ | ✓ | ✓ |
| `rotary_embedding` | 旋转位置嵌入 | ✓ | ✓ | ✓ |
| `attention_backend` | 注意力后端类路径 | ✓ | - | ✓ |

### 后端优先级

调度系统根据以下优先级层次结构选择算子。优先级值间隔 50，以便将来插入中间优先级。
