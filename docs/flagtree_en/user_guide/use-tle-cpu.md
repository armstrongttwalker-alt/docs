...
A unified programming paradigm covering diverse edge-side CPU compute power. Extends TLE's philosophy of "layering hardware deep optimizations on top of `@triton.jit`" to CPUs. Targeting the fragmented CPU ecosystem for edge AI (Arm64 / RISC-V / x86 with multiple coexisting ISAs), it uses the same Triton programming model + integration framework to support diverse ISAs: **programming model and correctness are shared across ISAs** (plain Triton lands on any ISA via LLVM), while each ISA's **high-performance implementation (intrinsic / C runtime) is contributed separately**. Model code is routed to the corresponding ISA implementation through the operator library (FlagGems) vendor dispatch, eliminating the need to be aware of the specific ISA. The differences between CPU and GPU dictate that it requires an independent extension surface:
...
- **Arm64**: The first fully implemented CPU backend, running LLM inference end-to-end (see sub-page).

- RISC-V (RVV) / x86 (AVX-512): Planned, reusing the same MLIR + TLE path.



================================================================

# Page 2: Arm64 (TLE-CPU Sub-page)
...
> External Links:
> 
> FlagTree https://github.com/flagos-ai/FlagTree (`triton_v3.3.x` branch, cpu backend)
> 
> flagtree-cpu https://github.com/flagos-ai/flagtree-cpu
> 
> FlagGems https://github.com/flagos-ai/FlagGems (`5.3.0-rc2`, `_arm` backend)
> 
> Installation Guide: FlagTree `documents/install_cpu.md`
> 
> 

### Overview

Arm64 is the first fully implemented backend for TLE-CPU, targeting the Armv9-A platform (NEON / SVE2 + i8mm + bf16), with reference hardware CIX P1 (CD8180, 8× Cortex-A720 big cores + 4× A520 little cores).

Software baseline: Triton 3.3 / LLVM a66376b0 / PyTorch 2.10 (CPU) / Python 3.11.

- **Compiler layer** (§2): Arm instruction selection and thread management optimizations upstreamable to Triton-CPU, making plain `tl.dot` correct and efficient on Arm.

- **Operator layer** (§3): Encapsulating decode hotspots into 6 TLE extension operations `create_cpu_*`.
...
#### Design Note: Dispatch Is Performance
