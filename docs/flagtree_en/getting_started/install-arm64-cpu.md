...
If `oaitriton.blob.core.windows.net` is reachable, the LLVM toolchain is fetched automatically on the first build (step 2.2 Step 3) according to `cmake/llvm-hash.txt` (a66376b0) and cached under `~/.triton/llvm/` — **no manual step needed**.
...
A: On heterogeneous SoCs (e.g. Cortex-A720 big + A520 little), always pin to the **big cores only** with `taskset` — little cores entering the OMP thread pool stall on the barrier and cost ~2x overall. Also set the governor to performance:
